...
function OrderList() {
  ...
  return (
    <Container>
      ...
      <Filter options={[{label: 'Nepasirinktas filtras', value: ''}, {label: 'Didėjant pagal datą', value: 'date'}, {label: 'Mažėjant pagal datą', value: '-date'}, {label: 'Didėjant pagal vardą', value: 'name'}, {label: 'Mažėjant pagal vardą', value: '-name'}]} name="Filtras" />
        </div>
      <div>
      <ItemsPerPageSelect options={[{label: '10', value: 10}, {label: '6', value: 6}, {label: '1', value: 1}]} name="Rodomų užsakymų skaičius" />
      </div>
      </div>
      {loading ?
       <div style={{ display: 'flex', height: '20vh', justifyContent: 'center' }}>
     ...
     <>
      ...
      <div style={{display: 'flex', justifyContent: 'center'}}>
        <Pagination pagesCount={Math.ceil(ordersCount / (itemsPerPage || 1))} selectedPage={+page}/>
      </div>
      </>
}
    </Container>
