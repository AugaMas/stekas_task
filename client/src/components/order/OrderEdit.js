...
function OrderEdit() {
  ...
  function prepareOrderObject(order) {
    return {
      ...
      products: order.products.map((p) => ({ ...p, count: +p.count })),
    };
  }
  ...
  function handleSubmit(e) {
    ...
    if (order.products.length) {
      if (id !== 'new') {
        axios
          .put(`/api/order/${id}`, prepareOrderObject(order), config)
          .then((res) => {
            history.push('/orders');
          })
          .catch((e) => {
        ...
        axios
          .post('/api/order/', prepareOrderObject(order), config)
          .then((res) => {
            history.push('/orders');
          })
          .catch((e) => {
      ...
      setError(
        `Negalima ${
          id !== 'new' ? 'redaguoti' : 'pateikti'
        } užsakymą nepridėjus prekių`
      );
    }
  }
      ...
      {loading ? (
        ...
        <>
          ...
          >
              ...
              >
                <Button
                  ...
                  onClick={() => {
                    setOrder({
                      ...order,
                      products: [...order.products, { name: '', count: 0 }],
                    });
                    setError('');
                  }}
                >
            ...
            {error && (
              <Form.Text style={{ color: 'red', marginBottom: '0.5em' }}>
                {error}
              </Form.Text>
            )}
